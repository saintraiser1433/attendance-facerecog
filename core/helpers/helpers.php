...
$test = getenv('FP_CLIENT_SERVICE_HOST');
error_log("ENV TEST: " . $test);
...
function enroll_fingerprint($pre_registered_fmd_array){
    ...
    try {
        ...
        // Log for debugging
        error_log("Enrollment service response: " . substr($response, 0, 200));
        
        return $response;
    } catch (\Exception $e) {
        error_log("Enrollment service error: " . $e->getMessage());
